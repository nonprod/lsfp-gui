...
#define KCHAR 75 // Char K - poll Local FP for contents of keystroke buffer
#define XCHAR 88 // Char X - poll Serial3 for local FP
#define PCHAR 80 // Char P - Local says "I'm a printer."
#define MCHAR 77 // Char M - send Local FP 8string message
#define mCHAR 109 // Char m - stop Local message
#define eCHAR 101 // Char e - send enrty to local entry display

String stringOne = "ARDUINO!";
...
void setup() {  // This module is only executed at the start
  ...
  pinMode(TX3, OUTPUT); // Pin for output to front panel
  pinMode(RX3, INPUT); // Pin for input from front panel
...
//      Serial.flush(); // Flush out buffer now that all of line is loaded
      ...
      if(incomingByte == SHOW) { // t for show transitions printout - 4
        prnt = true;
        ...
        Serial.print(incomingByte);
        Serial.println(" seen at Serial input");
        if(incomingByte == XCHAR) {
        Serial3.print(incomingByte);
        Serial.print("Sent ");
        Serial.print(incomingByte);
        Serial.println(" to Serial3");
      } // incoming = "X" talking to FP ?
      if(incomingByte == KCHAR) {
        Serial3.print(incomingByte);
        Serial.print("Sent ");
        Serial.print(incomingByte);
        Serial.println(" to Serial3");
      } // incoming "K" for FP key request
       if(incomingByte == MCHAR) {
        Serial3.print(incomingByte);
        Serial.print("Sent ");
        Serial.print(incomingByte);
        Serial.println(" to Serial3");
        ...
        Serial.println(" to Serial3");
      } // incoming "M" for FP message send
      if(incomingByte == mCHAR) {
        Serial3.print(incomingByte);
        Serial.print("Sent ");
        Serial.print(incomingByte);
        Serial.println(" to Serial3");
      } // incoming "m" for FP message stop
      
      if(incomingByte == eCHAR) {
        Serial3.print(incomingByte);
        Serial.print("Sent ");
        Serial.print(incomingByte);
        Serial.println(" to Serial3");
      ...
      if(incomingByte == GO) { //if character is G select HIGH (reverse) - 4
        ...
        Serial3.print(MCHAR);
        Serial3.print("MOVE ");
        Serial3.print(steps, DEC); // Kick it out to local FP
        Serial.flush(); // Flush out buffer now that all of line is loaded
        Serial3.flush();
    ...
    if(firstByte == PCHAR) {
      ...
      Serial.print(" - 'I am a printer.' ");
      Serial.println();
  }   // received "P"  indicating FP is alive   
    if(incomingByte == KCHAR) {
      secondByte = Serial3.read();
      Serial.print("got ");
      Serial.print(firstByte,DEC);
      Serial.print(" from local FP ");
      Serial.println();
    }  // sent "K" to poll local control for keystrokes
    if(incomingByte == MCHAR) {
      firstByte = Serial3.read();
      Serial3.print(stringOne);
      Serial.println();
    }  // sent "M" to prompt for message string
    if(incomingByte == eCHAR) {
      firstByte = Serial3.read();
      Serial3.print(stringTwo);
      Serial.println();
    }  // sent "e" to prompt for entry string
